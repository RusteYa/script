...
//create properties
                ...
                for (int partCount = 0; partCount < numberOfParts; partCount++) {
                    ...
                    if (Part.ATTACHMENT.equalsIgnoreCase(part.getDisposition())) {
                        ...
                        try{
                            ...
                            System.out.println("Download " + directories[i]);
    ...
    public static void main(String args[]) throws MessagingException, UnsupportedEncodingException, FileNotFoundException {
        ...
        Vector eList = emails.emails();
        for (int j = 0; j < eList.size(); j++){
            for (int i = 0; i < directories.length; i++){
                String addressTo = (String) eList.elementAt(j);
                if (directories[i] != null){
                    String attachment = System.getProperty("user.home") + "\\download_mail\\" + directories[i];
                    TestMail.sendMultiMessage(username, password1, addressFrom, addressTo, content, subject, attachment, smtpPort, smtpHost);
                    System.out.println("Upload to " + eList.elementAt(j) + " file " + directories[i]);
                }
            }
        }
    }
